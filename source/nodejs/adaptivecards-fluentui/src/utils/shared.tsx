...
export const sharedInternalRender = (renderReact: () => JSX.Element): HTMLElement => {
    const div = getDiv();
...
export abstract class ReactInputElement<TInput> extends AC.Input {

    private _value: string;
    private _inlineAction: AC.Action;
    protected abstract renderReact(): JSX.Element;
	protected internalInput: TInput;
...
export const raiseParseError = (event: AC.IValidationEvent, events: Array<AC.IValidationEvent>) => {
    if (events) {
        events.push(event);
    }
...
export const getIntValue = (val: any, errors?: AC.IValidationEvent[], defaultValue: number = undefined): number => {
    try {
