...
class SpinnerPeerInplaceEditor extends CardElementPeerInplaceEditor<Spinner> {
    ...
    applyChanges() {
        this.cardElement.size = this._renderedElement.value;
    }

    render() {
        this._renderedElement = document.createElement("textarea");
        this._renderedElement.className = "acd-spinner-inplace-editor";
        this._renderedElement.value = this.cardElement.size;
...
export class SpinnerPeer extends TypedCardElementPeer<Spinner> {
    static readonly sizeProperty = new StringPropertyEditor(Adaptive.Versions.v1_2, "size", "Size", true, true);

    protected createInplaceEditor(): DesignerPeerInplaceEditor {
        return new SpinnerPeerInplaceEditor(this.cardElement);
    }

    protected internalGetTreeItemText(): string {
        return this.cardElement.size;
    }
                ...
                new PeerCommand(
                    {
                        name: "Bind...",
                        alwaysShowName: true,
                        toolTip: "Select a data field to bind this spinner to.",
                        execute: (command: PeerCommand, clickedElement: HTMLElement) => {
                            ...
                            fieldPicker.onClose = (sender, wasCancelled) => {
                                if (!wasCancelled) {
                                    this.cardElement.size = fieldPicker.selectedField.asExpression();
    ...
    initializeCardElement() {
        if (!this.cardElement.size || this.cardElement.size == "") {
            this.cardElement.size = "medium";
        }
    }
}