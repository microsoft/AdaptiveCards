...
export class EvaluationContext {
	...
	static init() {
		...
		EvaluationContext._builtInFunctions["toUpper"] = (params: any[]) => {
			return (<string>params[0]).toUpperCase();
		};
		EvaluationContext._builtInFunctions["format"] = (params: any[]) => {
			switch (params[1]) {
				case ("%"):
		...
		EvaluationContext._builtInFunctions["parseDateFromEpoch"] = (params: any[]) => {
			try {
