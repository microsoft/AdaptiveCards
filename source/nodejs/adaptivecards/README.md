...
|| The `AdaptiveCard.onParseError` event has been **REMOVED**. Parse errors are now collected into the `CardObjectParseContext.errors` property. ||
|| The `AdaptiveCard.onParseElement` and `AdaptiveCard.onParseAction` events (both static and instance versions) have been **REMOVED**. Use `CardObjectParseContext.onParseElement` and `CardObjectParseContext.onParseAction` instead. ||
