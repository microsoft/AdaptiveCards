...
|| The `AdaptiveCard.onParseError` event has been **REMOVED**. Parse errors are now collected into the `ParseContext.errors` property. ||
|| The `AdaptiveCard.onParseElement` and `AdaptiveCard.onParseAction` events (both static and instance versions) have been **REMOVED**. Use `ParseContext.onParseElement` and `ParseContext.onParseAction` instead. |
|| The `createActionInstance` and `createElementInstance` global functions have been **REMOVED** and replaced with the `parseAction` and `parseElement` methods of the `ParseContext` class. |
|| The signature of the `CardObject.parse` method has changed to `parse(source: any, context?: ParseContext)`|
...
|| Serialization and deserialization is now done using a "schema" model, where each property of a serializable object is declared as a `PropertyDefinition` instance. Class members that expose those properties are identified via the use of the `@property` decorator. The PropertyDefinition model makes it possible to associate version information with pretty much eveything, which allows the library to (de)serialize to/from any version of the Adaptive Card schema. |
