...
export class AdaptiveApplet {
    ...
    private showManualRefreshButton(refreshAction: ExecuteAction) {
        let displayBuiltInManualRefreshButton = this.onShowManualRefreshButton
        ...
        if (displayBuiltInManualRefreshButton) {
            ...
            if (this.onRenderManualRefreshButton) {
                ...
                if (
                    ...
                    let autoRefreshPausedMessage =
                        Strings.runtime.automaticRefreshPaused();

                    if (
                        autoRefreshPausedMessage[
                            autoRefreshPausedMessage.length - 1
                        ] !== " "
                    ) {
                        autoRefreshPausedMessage += " ";
                    }
            ...
            if (renderedRefreshButton) {
                ...
                this._refreshButtonHostElement.appendChild(
                    renderedRefreshButton
                );
    ...
    private createActivityRequest(
        ...
        if (this.card) {
            let request = new ActivityRequest(
                action,
                trigger,
            ...
            let cancel = this.onPrepareActivityRequest
    ...
    private internalSetCard(payload: any, consecutiveRefreshes: number) {
        ...
        if (this._cardPayload) {
            try {
                ...
                let doChangeCard = this.onCardChanging
                    ...
                    if (this._card.renderedElement) {
                        ...
                        if (this._card.refresh) {
                            if (
                                ...
                                if (
                                    GlobalSettings.applets.refresh
                                        .timeBetweenAutomaticRefreshes <= 0
                                ) {
                                    logEvent(
                                        Enums.LogLevel.Info,
                                ...
                                if (
                                    ...
                                    GlobalSettings.applets.refresh.mode ===
                                        Enums.RefreshMode.Manual
                                ) {
                                    logEvent(
                                        Enums.LogLevel.Info,
                                        "Showing manual refresh button."
                                    );
    ...
    private internalExecuteAction(
        ...
        if (action instanceof ExecuteAction) {
            if (this.channelAdapter) {
                let request = this.createActivityRequest(
                    action,
                    trigger,
                    consecutiveRefreshes
                );
                ...
                throw new Error(
                    "internalExecuteAction: No channel adapter set."
                );
            }
        }
    ...
    private createProgressOverlay(
        ...
        if (!this._progressOverlay) {
            if (this.onCreateProgressOverlay) {
                this._progressOverlay = this.onCreateProgressOverlay(
                    this,
                    request
                );
            } else {
    ...
    private showAuthCodeInputDialog(request: ActivityRequest) {
        let showBuiltInAuthCodeInputCard = this.onShowAuthCodeInputDialog
            ? this.onShowAuthCodeInputDialog(this, request)
            : true;

        if (showBuiltInAuthCodeInputCard) {
            let authCodeInputCard = this.createMagicCodeInputCard(
                request.attemptNumber
            );
            authCodeInputCard.render();
            authCodeInputCard.onExecuteAction = (
                submitMagicCodeAction: Action
            ) => {
                if (
                    this.card &&
                    submitMagicCodeAction instanceof SubmitAction
                ) {
                    switch (submitMagicCodeAction.id) {
                        case AdaptiveApplet.submitMagicCodeActionId:
                            ...
                            if (
                                ...
                                typeof (<any>submitMagicCodeAction.data)[
                                    "magicCode"
                                ] === "string"
                            ) {
                                authCode = (<any>submitMagicCodeAction.data)[
                                    "magicCode"
                                ];
                            }
                            ...
                            if (authCode) {
                                ...
                                alert(
                                    "Please enter the magic code you received."
                                );
                            }
                        ...
                        default:
                            logEvent(
                                ...
                                "Unespected action taken from magic code input card (id = " +
                                    submitMagicCodeAction.id +
                                    ")"
                            );

                            alert(
                                Strings.magicCodeInputCard.somethingWentWrong()
                            );
    ...
    private async internalSendActivityRequestAsync(request: ActivityRequest) {
        if (!this.channelAdapter) {
            throw new Error(
                "internalSendActivityRequestAsync: channelAdapter is not set."
            );
        }
        ...
        while (!done) {
            ...
            if (request.attemptNumber === 1) {
                logEvent(
                    Enums.LogLevel.Info,
                    "Sending activity request to channel (attempt " +
                        request.attemptNumber +
                        ")"
                );
            } else {
                logEvent(
                    Enums.LogLevel.Info,
                    "Re-sending activity request to channel (attempt " +
                        request.attemptNumber +
                        ")"
                );
            }
            ...
            if (response) {
                if (response instanceof SuccessResponse) {
                    ...
                    if (typeof parsedContent === "string") {
                        ...
                        this.internalSetCard(
                            parsedContent,
                            request.consecutiveRefreshes
                        );
                    ...
                    if (
                        retryIn >= 0 &&
                        request.attemptNumber <
                            GlobalSettings.applets.maximumRetryAttempts
                    ) {
                        logEvent(
                            Enums.LogLevel.Warning,
                    ...
                    if (
                        ...
                        alert(
                            Strings.magicCodeInputCard.authenticationFailed()
                        );
                    }
                    ...
                    throw new Error(
                        "Unhandled response type: " + response.toString()
                    );
                }
            }
        }
    ...
    onRemoveProgressOverlay?: (
        sender: AdaptiveApplet,
        request: IActivityRequest
    ) => void;
    onRenderManualRefreshButton?: (
        sender: AdaptiveApplet
    ) => HTMLElement | undefined;
