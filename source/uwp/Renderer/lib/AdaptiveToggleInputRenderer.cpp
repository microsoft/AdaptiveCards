...
Xaml::UIElement AdaptiveToggleInputRenderer::Render(IAdaptiveCardElement cardElement, AdaptiveRenderContext renderContext, AdaptiveRenderArgs renderArgs)
{
    if (!renderContext.HostConfig().SupportsInteractivity())
    {
        renderContext.AddWarning(
            winrt::AdaptiveCards::ObjectModel::Uwp::WarningStatusCode::InteractivityNotSupported,
    ...
    checkBox.Tapped(
        [](IInspectable const& /* sender */, Xaml::Input::TappedRoutedEventArgs const& args) { return args.Handled(true); });
