...
try
{
    ...
    HRESULT hrColumns = IterateOverVectorWithFailure<AdaptiveColumn, IAdaptiveColumn>(columns.Get(), ancestorHasFallback, [&](IAdaptiveColumn* column) {
        ...
        {
            RETURN_IF_FAILED(XamlHelpers::RenderFallback(
                columnAsCardElement.Get(), renderContext, newRenderArgs.Get(), &xamlColumn, nullptr));
        }
    ...
    RETURN_IF_FAILED(XamlHelpers::SetStyleFromResourceDictionary(
        renderContext, L"Adaptive.ColumnSet", columnSetAsFrameworkElement.Get()));
