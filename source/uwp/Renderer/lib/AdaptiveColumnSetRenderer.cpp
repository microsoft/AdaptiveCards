...
try
{
    ...
    ComPtr<IBorder> columnSetBorder = XamlHelpers::CreateABIClass<IBorder>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Border));
    ...
    HRESULT hrColumns = IterateOverVectorWithFailure<AdaptiveColumn, IAdaptiveColumn>(columns.Get(), ancestorHasFallback, [&](IAdaptiveColumn* column) {
        ...
        if (xamlColumn != nullptr)
        {
            ...
            RETURN_IF_FAILED(XamlHelpers::AddRenderedControl(
                xamlColumn.Get(), columnAsCardElement.Get(), gridAsPanel.Get(), separator.Get(), columnDefinition.Get(), [](IUIElement*) {}));
        }
        return S_OK;
    });
    ...
    RETURN_IF_FAILED(XamlHelpers::SetStyleFromResourceDictionary(renderContext, L"Adaptive.ColumnSet", columnSetAsFrameworkElement.Get()));
