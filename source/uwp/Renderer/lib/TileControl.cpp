...
try
{
    ...
    RETURN_IF_FAILED(spFactory->CreateInstance(
        static_cast<ITileControl*>(this), spInnerInspectable.GetAddressOf(), spInnerContentControl.GetAddressOf()));
    RETURN_IF_FAILED(SetComposableBasePointers(spInnerInspectable.Get(), spFactory.Get()));

    // initialize members
    m_containerElement = XamlHelpers::CreateABIClass<ICanvas>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Canvas));
...
HRESULT TileControl::get_ResolvedImage(_COM_Outptr_ IUIElement** value)
{
...
HRESULT TileControl::MeasureOverride(Size availableSize, Size* pReturnValue)
{
    ...
    RETURN_IF_FAILED(GetComposableBase()->QueryInterface(__uuidof(IFrameworkElementOverrides), reinterpret_cast<void**>(base.GetAddressOf())));
...
void TileControl::RefreshContainerTile()
{
    ...
    for (int x = 0, index = 0; x < numberImagePerRow; x++)
    {
        for (int y = 0; y < numberImagePerColumn; y++, index++)
        {
            ...
            ABI::Windows::Foundation::GetActivationFactory(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Canvas).Get(), &canvasStatics);
