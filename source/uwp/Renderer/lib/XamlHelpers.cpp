    ...
    _In_ IAdaptiveRenderContext* renderContext, UINT spacing, UINT separatorThickness, ABI::Windows::UI::Color separatorColor, bool isHorizontal)
{
    ...
    THROW_IF_FAILED(XamlHelpers::SetStyleFromResourceDictionary(
        renderContext, L"Adaptive.Separator", separatorAsFrameworkElement.Get()));
...
void ApplyBackgroundToRoot(
    _In_ IPanel* rootPanel, _In_ IAdaptiveBackgroundImage* backgroundImage, _In_ IAdaptiveRenderContext* renderContext, _In_ IAdaptiveRenderArgs* renderArgs)
{
    ...
    else
    {
        ...
        if (!ancestorHasFallback)
        {
            ...
            RETURN_IF_FAILED(renderContext->AddWarning(
                ABI::AdaptiveCards::ObjectModel::Uwp::WarningStatusCode::NoRendererForType,
...
HRESULT FormatErrorMessageWithHostConfig(_In_ ABI::AdaptiveCards::Rendering::Uwp::IAdaptiveRenderContext* renderContext, ITextBlock* errorMessage)
{
    ...
    UINT32 resultSize{};
    RETURN_IF_FAILED(GetFontSizeFromFontType(
        hostConfig.Get(), ABI::AdaptiveCards::ObjectModel::Uwp::FontType_Default, textSize, &resultSize));
    ...
    RETURN_IF_FAILED(GetFontWeightFromStyle(
        hostConfig.Get(), ABI::AdaptiveCards::ObjectModel::Uwp::FontType_Default, textWeight, &resultWeight));
    ...
    IBorder** validationBorderOut)
{
    ...
    // The AutomationProperties.IsRequiredForForm property allows an input to provide a little bit of extra information
    // to people using a screen reader by specifying if an input is required. Visually we represent this with a hint.
    ...
    if (SUCCEEDED(localAdaptiveInput.As(&adaptiveToggleInput)))
    {
        ...
        RETURN_IF_FAILED(automationPropertiesStatics5->GetDescribedBy(
