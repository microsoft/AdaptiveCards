...
HRESULT AdaptiveContainerRenderer::RuntimeClassInitialize() noexcept
{
    return S_OK;
}
...
try
{
    ...
    if (currentRtl)
    {
        ...
        RETURN_IF_FAILED(containerPanelAsFrameWorkElement->put_FlowDirection(rtlValue ? FlowDirection_RightToLeft : FlowDirection_LeftToRight));
    }
    ...
    ComPtr<IBorder> containerBorder = XamlHelpers::CreateABIClass<IBorder>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Border));
