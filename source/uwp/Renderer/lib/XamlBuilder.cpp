...
XamlBuilder::XamlBuilder()
{
    ...
    THROW_IF_FAILED(GetActivationFactory(
        HStringReference(RuntimeClass_Windows_Storage_Streams_RandomAccessStream).Get(), &m_randomAccessStreamStatics));
}
...
HRESULT XamlBuilder::BuildXamlTreeFromAdaptiveCard(
...
try
{
    *xamlTreeRoot = nullptr;
    if (adaptiveCard != nullptr)
    {
        ...
        RETURN_IF_FAILED(CreateRootCardElement(
            adaptiveCard, renderContext, renderArgs.Get(), xamlBuilder, &bodyElementContainer, &rootElement));
        ...
        ActionHelpers::HandleSelectAction(
            nullptr, selectAction.Get(), renderContext, rootElement.Get(), XamlHelpers::SupportsInteractivity(hostConfig.Get()), true, &rootSelectActionElement);
        ...
        RETURN_IF_FAILED(MakeAndInitialize<AdaptiveRenderArgs>(
            &bodyRenderArgs, containerStyle, rootAsFrameworkElement.Get(), adaptiveCard, nullptr));
        RETURN_IF_FAILED(BuildPanelChildren(
            body.Get(), bodyElementContainer.Get(), renderContext, bodyRenderArgs.Get(), [](IUIElement*) {}));
        ...
        if (actionsSize > 0)
        {
            if (XamlHelpers::SupportsInteractivity(hostConfig.Get()))
            {
                ...
                ActionHelpers::BuildActions(
                    adaptiveCard, actions.Get(), bodyElementContainer.Get(), bodyCount > 0, renderContext, renderArgs.Get());
            }
            else
            {
        ...
        else
        {
            RETURN_IF_FAILED(XamlHelpers::SetStyleFromResourceDictionary(
                renderContext, L"Adaptive.ShowCard.Card", rootAsFrameworkElement.Get()));
        }
...
HRESULT XamlBuilder::CreateRootCardElement(
    _In_ IAdaptiveCard* adaptiveCard,
    _In_ IAdaptiveRenderContext* renderContext,
...
try
{
    ...
    ComPtr<IGrid> rootElement = XamlHelpers::CreateABIClass<IGrid>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Grid));
    ComPtr<IAdaptiveHostConfig> hostConfig;
    RETURN_IF_FAILED(renderContext->get_HostConfig(&hostConfig));
    ComPtr<IAdaptiveCardConfig> adaptiveCardConfig;
    RETURN_IF_FAILED(hostConfig->get_AdaptiveCard(&adaptiveCardConfig));
...
HRESULT XamlBuilder::BuildPanelChildren(
    _In_ IVector<IAdaptiveCardElement*>* children,
    ...
    std::function<void(IUIElement* child)> childCreatedCallback) noexcept
{
    ...
    HRESULT hr = IterateOverVectorWithFailure<IAdaptiveCardElement>(children, ancestorHasFallback, [&](IAdaptiveCardElement* element) {
        ...
        if (newControl != nullptr)
        {
            ...
            RETURN_IF_FAILED(XamlHelpers::AddRenderedControl(
                newControl.Get(), element, parentPanel, separator.Get(), nullptr, childCreatedCallback));
        }
