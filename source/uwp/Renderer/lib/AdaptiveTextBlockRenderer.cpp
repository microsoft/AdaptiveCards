...
try
{
    ...
    if (text.Get() == nullptr)
    {
        *textBlockControl = nullptr;
        renderContext->AddError(
            ABI::AdaptiveCards::ObjectModel::Uwp::ErrorStatusCode::RequiredPropertyMissing,
    ...
    RETURN_IF_FAILED(XamlHelpers::SetStyleFromResourceDictionary(renderContext, L"Adaptive.TextBlock", frameworkElement.Get()));
...
HRESULT AdaptiveTextBlockRenderer::GetHeadingLevelFromContext(
