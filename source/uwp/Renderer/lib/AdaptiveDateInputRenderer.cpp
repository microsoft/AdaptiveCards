...
try
{
    ...
    ComPtr<ICalendarDatePicker> datePicker =
        XamlHelpers::CreateABIClass<ICalendarDatePicker>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_CalendarDatePicker));
    ...
    if (DateTimePreparser::TryParseSimpleDate(max, year, month, day))
    {
        ...
        if (isMinValid)
        {
            ...
            {
                renderContext->AddWarning(
                    ABI::AdaptiveCards::ObjectModel::Uwp::WarningStatusCode::InvalidValue,
    ...
    RETURN_IF_FAILED(MakeAndInitialize<DateInputValue>(&input, adaptiveDateInput.Get(), datePicker.Get(), validationBorder.Get()));
