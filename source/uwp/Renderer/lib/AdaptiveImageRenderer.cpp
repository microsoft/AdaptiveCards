...
HRESULT XamlBuilder::BuildImage(
    ...
    _COM_Outptr_ IUIElement** imageControl)
{
    ...
    if (imageUrl == nullptr)
    {
        renderContext->AddWarning(
            ABI::AdaptiveCards::ObjectModel::Uwp::WarningStatusCode::AssetLoadFailed, HStringReference(L"Image not found").Get());
    ...
    {
        ComPtr<IEllipse> ellipse = XamlHelpers::CreateABIClass<IEllipse>(HStringReference(RuntimeClass_Windows_UI_Xaml_Shapes_Ellipse));
        ...
        if (size == ABI::AdaptiveCards::ObjectModel::Uwp::ImageSize::None || size == ABI::AdaptiveCards::ObjectModel::Uwp::ImageSize::Stretch ||
        ...
        if (backgroundColor.Get() != nullptr)
        {
            ...
            ComPtr<IGrid> imageGrid = XamlHelpers::CreateABIClass<IGrid>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Grid));
    ...
    {
        ComPtr<IImage> xamlImage = XamlHelpers::CreateABIClass<IImage>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Image));

        if (backgroundColor != nullptr)
        {
            // Create a surrounding border with solid color background to contain the image
            ComPtr<IBorder> border = XamlHelpers::CreateABIClass<IBorder>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Border));
    ...
    else
    {
        if (size == ABI::AdaptiveCards::ObjectModel::Uwp::ImageSize::Small ||
            size == ABI::AdaptiveCards::ObjectModel::Uwp::ImageSize::Medium || size == ABI::AdaptiveCards::ObjectModel::Uwp::ImageSize::Large)
        {
            UINT32 imageSize;
            switch (size)
            {
    ...
    ActionHelpers::HandleSelectAction(
        adaptiveCardElement,
        selectAction.Get(),
        renderContext,
...
void XamlBuilder::PopulateImageFromUrlAsync(_In_ IUriRuntimeClass* imageUrl, _In_ T* imageControl)
{
    ...
    THROW_IF_FAILED(GetActivationFactory(
        HStringReference(RuntimeClass_Windows_Web_Http_HttpClient).Get(), httpClientFactory.ReleaseAndGetAddressOf()));
...
template <>
void XamlBuilder::SetImageSource<IEllipse>(_In_ IEllipse* destination, _In_ IImageSource* imageSource, ABI::Windows::UI::Xaml::Media::Stretch stretch)
{
                ...
                Callback<IRoutedEventHandler>(
                    [ellipseAsFrameworkElement, weakParent, isVisible](
                        ...
                        if (isVisible)
                        {
                            ...
                            {
                                RETURN_IF_FAILED(XamlHelpers::SetAutoImageSize(
                                    ellipseAsFrameworkElement.Get(), lambdaParentElement.Get(), lambdaImageSourceAsBitmap.Get(), isVisible));
                            }
                        }
                        return S_OK;
                    })
            ...
            THROW_IF_FAILED(xamlImage->add_ImageOpened(
                Callback<IRoutedEventHandler>(
                    [weakImage, weakParent, imageSourceAsBitmap, isVisible](
                        ...
                        {
                            RETURN_IF_FAILED(XamlHelpers::SetAutoImageSize(
                                lambdaImageAsFrameworkElement.Get(), lambdaParentElement.Get(), imageSourceAsBitmap.Get(), isVisible));
                        }
                        return S_OK;
                    })
                    .Get(),
