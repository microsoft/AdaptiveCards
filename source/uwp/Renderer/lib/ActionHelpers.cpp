...
void SetTooltip(_In_opt_ HSTRING toolTipText, _In_ IDependencyObject* tooltipTarget)
{
    if (toolTipText != nullptr)
    {
        ...
        ComPtr<IToolTip> toolTip = XamlHelpers::CreateABIClass<IToolTip>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_ToolTip));
    ...
    _In_ IButton* button)
{
    ...
    if (iconUrl != nullptr)
    {
        ...
        THROW_IF_FAILED(MakeAndInitialize<AdaptiveRenderArgs>(&childRenderArgs, containerStyle, buttonContentsStackPanel.Get(), nullptr));
...
HRESULT HandleActionStyling(
    _In_ IAdaptiveActionElement* adaptiveActionElement,
    ...
    _In_ IAdaptiveRenderContext* renderContext)
{
    ...
    {
        if (SUCCEEDED(XamlHelpers::TryGetResourceFromResourceDictionaries<IStyle>(
                resourceDictionary.Get(), L"Adaptive.Action.Overflow", &styleToApply)))
        {
    ...
    else if (SUCCEEDED(WindowsCompareStringOrdinal(actionSentiment.Get(), HStringReference(L"positive").Get(), &isSentimentPositive)) && (isSentimentPositive == 0))
    {
        if (SUCCEEDED(XamlHelpers::TryGetResourceFromResourceDictionaries<IStyle>(
                resourceDictionary.Get(), L"Adaptive.Action.Positive", &styleToApply)))
        {
            RETURN_IF_FAILED(buttonFrameworkElement->put_Style(styleToApply.Get()));
        }
        else
        {
            ...
            if (SUCCEEDED(XamlHelpers::TryGetResourceFromResourceDictionaries(
                    actionSentimentDictionary.Get(), L"PositiveActionDefaultStyle", styleToApply.GetAddressOf())))
            {
                RETURN_IF_FAILED(buttonFrameworkElement->put_Style(styleToApply.Get()));
            }
        }
    }
    else if (SUCCEEDED(WindowsCompareStringOrdinal(actionSentiment.Get(), HStringReference(L"destructive").Get(), &isSentimentDestructive)) && (isSentimentDestructive == 0))
    {
        if (SUCCEEDED(XamlHelpers::TryGetResourceFromResourceDictionaries<IStyle>(
                resourceDictionary.Get(), L"Adaptive.Action.Destructive", &styleToApply)))
        {
            RETURN_IF_FAILED(buttonFrameworkElement->put_Style(styleToApply.Get()));
        }
        else
        {
            ...
            if (SUCCEEDED(XamlHelpers::TryGetResourceFromResourceDictionaries(
                    actionSentimentDictionary.Get(), L"DestructiveActionDefaultStyle", styleToApply.GetAddressOf())))
            {
                RETURN_IF_FAILED(buttonFrameworkElement->put_Style(styleToApply.Get()));
            }
        }
    ...
    else
    {
        ...
        RETURN_IF_FAILED(WindowsConcatString(
            HStringReference(L"Adaptive.Action.").Get(), actionSentiment.Get(), actionSentimentStyle.GetAddressOf()));
    ...
    _Outptr_ IUIElement** actionControl)
{
    ...
    if (actionsOrientation == ABI::AdaptiveCards::Rendering::Uwp::ActionsOrientation::Horizontal)
    {
        ...
        RETURN_IF_FAILED(buttonFrameworkElement->put_HorizontalAlignment(ABI::Windows::UI::Xaml::HorizontalAlignment::HorizontalAlignment_Stretch));
    }
    else
    {
    ...
    if (action != nullptr)
    {
        ...
        if (actionType == ABI::AdaptiveCards::ObjectModel::Uwp::ActionType::ShowCard)
        {
            THROW_IF_FAILED(renderContext->AddWarning(
                ABI::AdaptiveCards::ObjectModel::Uwp::WarningStatusCode::UnsupportedValue, HStringReference(warning.c_str()).Get()));
            return true;
        }
    }
...
static HRESULT HandleKeydownForInlineAction(_In_ IKeyRoutedEventArgs* args, _In_ IAdaptiveActionInvoker* actionInvoker, _In_ IAdaptiveActionElement* inlineAction)
{
    ...
    if (key == ABI::Windows::System::VirtualKey::VirtualKey_Enter)
    {
        ...
        if (shiftKeyState == ABI::Windows::UI::Core::CoreVirtualKeyStates_None && ctrlKeyState == ABI::Windows::UI::Core::CoreVirtualKeyStates_None)
        {
    ...
    _COM_Outptr_ IUIElement** textBoxWithInlineAction)
{
    ...
    ComPtr<IColumnDefinition> textBoxColumnDefinition =
        XamlHelpers::CreateABIClass<IColumnDefinition>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_ColumnDefinition));
    ...
    ComPtr<IColumnDefinition> separatorColumnDefinition =
        XamlHelpers::CreateABIClass<IColumnDefinition>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_ColumnDefinition));
    ...
    ComPtr<IColumnDefinition> inlineActionColumnDefinition =
        XamlHelpers::CreateABIClass<IColumnDefinition>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_ColumnDefinition));
    ...
    else
    {
        ...
        ComPtr<IGrid> titleGrid = XamlHelpers::CreateABIClass<IGrid>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Grid));
        ComPtr<IPanel> panel;
    ...
    WrapInTouchTarget(
        nullptr, actionUIElement.Get(), localInlineAction.Get(), renderContext, false, L"Adaptive.Input.Text.InlineAction", nullptr, (iconUrl != nullptr), &touchTargetUIElement);
    ...
    _COM_Outptr_ IUIElement** finalElement)
{
    ...
    THROW_IF_FAILED(XamlHelpers::SetStyleFromResourceDictionary(renderContext, style.c_str(), buttonAsFrameworkElement.Get()));
    ...
    _COM_Outptr_ IUIElement** outUiElement)
{
    ...
    {
        WrapInTouchTarget(
            adaptiveCardElement, uiElement, selectAction, renderContext, fullWidthTouchTarget, L"Adaptive.SelectAction", nullptr, true, outUiElement);
    }
    else
    {
...
HRESULT BuildActions(
    _In_ IAdaptiveCard* adaptiveCard,
...
HRESULT CreateFlyoutButton(_In_ IAdaptiveRenderContext* renderContext, _In_ IAdaptiveRenderArgs* renderArgs, _COM_Outptr_ IButton** overflowButton)
{
...
HRESULT BuildInlineShowCard(
    _In_opt_ ABI::AdaptiveCards::ObjectModel::Uwp::IAdaptiveCard* adaptiveCard,
    ...
    _In_ IAdaptiveRenderArgs* renderArgs)
{
    ...
    if (showCardActionMode == ABI::AdaptiveCards::Rendering::Uwp::ActionMode::Inline)
    {
        ...
        {
            RETURN_IF_FAILED(contextImpl->AddInlineShowCard(adaptiveActionSet, actionAsShowCardAction.Get(), uiShowCard.Get(), renderArgs));
        }
        else
        {
...
HRESULT CreateActionButtonInActionSet(
    _In_opt_ IAdaptiveCard* adaptiveCard,
    _In_opt_ IAdaptiveActionSet* adaptiveActionSet,
    ...
    while (!renderer)
    {
        ...
        if (!renderer)
        {
            ...
            case ABI::AdaptiveCards::ObjectModel::Uwp::FallbackType::Content:
            {
                ...
                RETURN_IF_FAILED(
                    XamlHelpers::WarnForFallbackContentElement(renderContext, actionTypeString.Get(), fallbackTypeString.Get()));
    ...
    _Outptr_ IUIElement** actionSetControl)
{
        ...
        actionsOrientation == ABI::AdaptiveCards::Rendering::Uwp::ActionsOrientation::Horizontal)
    {
        ...
        ComPtr<IGrid> actionsGrid = XamlHelpers::CreateABIClass<IGrid>(HStringReference(RuntimeClass_Windows_UI_Xaml_Controls_Grid));
    ...
    IterateOverVector<IAdaptiveActionElement>(children, [&](IAdaptiveActionElement* child) {
        ...
        if (currentButtonIndex < maxActions && mode == ABI::AdaptiveCards::ObjectModel::Uwp::ActionMode::Primary)
        {
            ...
            RETURN_IF_FAILED(CreateActionButtonInActionSet(
                adaptiveCard,
                adaptiveActionSet,
                action.Get(),
                currentButtonIndex,
        ...
        else if (currentButtonIndex >= maxActions && (mode == ABI::AdaptiveCards::ObjectModel::Uwp::ActionMode::Primary) && !overflowMaxActions)
        {
            ...
            renderContext->AddWarning(
                ABI::AdaptiveCards::ObjectModel::Uwp::WarningStatusCode::MaxActionsExceeded,
