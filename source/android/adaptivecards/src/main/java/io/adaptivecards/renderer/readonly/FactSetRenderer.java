...
public class FactSetRenderer extends BaseCardElementRenderer
{
    ...
    static TextView createTextView(Context context, RenderArgs renderArgs, CharSequence text, FactSetTextConfig textConfig, HostConfig hostConfig, long spacing, ContainerStyle containerStyle)
    {
        ...
        TextBlockRenderer.applyTextColor(textView, hostConfig, renderArgs, TextStyle.Default, textConfig.getColor(), textConfig.getIsSubtle(), containerStyle);
        TextBlockRenderer.applyTextSize(textView, hostConfig, renderArgs,TextStyle.Default, FontType.Default, textConfig.getSize());
        TextBlockRenderer.applyTextFormat(textView, hostConfig, renderArgs, TextStyle.Default, FontType.Default, textConfig.getWeight());
            ...
            RenderArgs renderArgs) throws Exception
    {
        ...
        for (int i = 0; i < factVectorSize; i++)
        {
            ...
            if (!titleWithFormattedDates.isEmpty() || !valueWithFormattedDates.isEmpty())
            {
                ...
                factRow.addView(createTextView(context, renderArgs, RendererUtil.handleSpecialText(titleWithFormattedDates), hostConfig.GetFactSet().getTitle(), hostConfig, spacing, renderArgs.getContainerStyle()));
                factRow.addView(createTextView(context, renderArgs, RendererUtil.handleSpecialText(valueWithFormattedDates), hostConfig.GetFactSet().getValue(), hostConfig, 0, renderArgs.getContainerStyle()));
