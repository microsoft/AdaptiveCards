         ...
         hostConfig:(ACOHostConfig *)acoConfig;
{
    ...
    ACRColumnSetView *columnSetView =
        [[ACRColumnSetView alloc] initWithStyle:(ACRContainerStyle)columnSetElem->GetStyle()
    ...
    [columnSetView setContentCompressionResistancePriority:UILayoutPriorityRequired
                                                   forAxis:UILayoutConstraintAxisVertical];

    ACRBaseCardElementRenderer *columnRenderer = [[ACRRegistration getInstance]
        getRenderer:[NSNumber numberWithInt:(int)CardElementType::Column]];
    ...
    if (columnSetElem->GetMinHeight() > 0) {
        [constraints
            addObject:[NSLayoutConstraint constraintWithItem:columnSetView
    ...
    for (std::shared_ptr<Column> column : columns) {
        if (*firstColumn != column) {
            ACRSeparator *separator = [ACRSeparator renderSeparation:column
                                                        forSuperview:columnSetView
                                                      withHostConfig:config];
            configSeparatorVisibility(separator, prevColumn);
        }
        [acoColumn setElem:column];
        curView = (ACRColumnView *)[columnRenderer render:columnSetView
                                                 rootView:rootView
                                                   inputs:inputs
                                          baseCardElement:acoColumn
                                               hostConfig:acoConfig];

        // when stretch, views with stretch properties should have equal width
        if (curView.pixelWidth) {
            [constraints
                addObject:[NSLayoutConstraint constraintWithItem:curView
            ...
            if (stretchView) {
                [constraints addObject:[NSLayoutConstraint constraintWithItem:curView
                                                                    attribute:NSLayoutAttributeWidth
                                                                    relatedBy:NSLayoutRelationEqual
                                                                       toItem:stretchView
                                                                    attribute:NSLayoutAttributeWidth
            ...
            //[curView setContentHuggingPriority:UILayoutPriorityDefaultLow
            //                           forAxis:UILayoutConstraintAxisHorizontal];
            //[curView setContentCompressionResistancePriority:UILayoutPriorityDefaultLow
              //                                       forAxis:UILayoutConstraintAxisHorizontal];

            stretchView = curView;
        } else if ([curView.columnWidth isEqualToString:@"auto"]) {
            //[curView setContentHuggingPriority:UILayoutPriorityRequired
            //                           forAxis:UILayoutConstraintAxisHorizontal];
            //[curView setContentCompressionResistancePriority:UILayoutPriorityDefaultHigh
                                                     //forAxis:UILayoutConstraintAxisHorizontal];
        } else {
            try {
                ...
                if (curView.hasStretchableView ||
                    (columnSetElem->GetHeight() == HeightType::Stretch)) {
