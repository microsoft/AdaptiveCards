...
@implementation ACRRendererInternal
                         ...
                         hostconfig:(std::shared_ptr<HostConfig> const &)config
{
    ...
    if(!body.empty())
    {
        ...
        [ACRRendererInternal render:verticalView inputs:inputs withCardElems:body andHostConfig:config];

        std::vector<std::shared_ptr<BaseActionElement>> actions = adaptiveCard->GetActions();
        UIView<ACRIContentHoldingView> *actionChildView = [ACRRendererInternal renderButton:vc inputs:inputs superview:verticalView actionElems:actions hostConfig:config];
