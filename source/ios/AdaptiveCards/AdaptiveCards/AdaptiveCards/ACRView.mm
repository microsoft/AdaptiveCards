...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _serial_queue =
            dispatch_queue_create("io.adaptiveCards.serial_queue", DISPATCH_QUEUE_SERIAL);
        _serial_text_queue =
            dispatch_queue_create("io.adaptiveCards.serial_text_queue", DISPATCH_QUEUE_SERIAL);
...
- (instancetype)init:(ACOAdaptiveCard *)card
    ...
    if (self) {
        ...
        _actionsTargetBuilderDirector = [[ACRTargetBuilderDirector alloc] init:self
                                                                    capability:ACRAction
                                                            adaptiveHostConfig:_hostConfig];
        _selectActionsTargetBuilderDirector = [[ACRTargetBuilderDirector alloc] init:self
                                                                          capability:ACRSelectAction
                                                                  adaptiveHostConfig:_hostConfig];
        _quickReplyTargetBuilderDirector = [[ACRTargetBuilderDirector alloc] init:self
                                                                       capability:ACRQuickReply
                                                               adaptiveHostConfig:_hostConfig];
...
- (UIView *)render {
    ...
    ContainerStyle style = ([_hostConfig getHostConfig] -> GetAdaptiveCard().allowCustomStyle)
        ? [_adaptiveCard card] -> GetStyle()
        : ContainerStyle::Default;

    newView.backgroundColor = [_hostConfig
        getBackgroundColorForContainerStyle:[ACOHostConfig getPlatformContainerStyle:style]];
...
- (void)processBaseCardElement:(std::shared_ptr<BaseCardElement> const &)elem {
    switch (elem->GetElementType()) {
        case CardElementType::TextBlock: {
            ...
            NSNumber *number =
                [NSNumber numberWithUnsignedLongLong:(unsigned long long)textBlockElement.get()];
            NSString *key = [number stringValue];
            [self processTextConcurrently:textProp elementId:key];
            break;
        }
        case CardElementType::RichTextBlock: {
            std::shared_ptr<RichTextBlock> rTxtBlkElement =
                std::static_pointer_cast<RichTextBlock>(elem);
        ...
        case CardElementType::FactSet: {
            ...
            NSString *key = [NSString stringWithCString:elem->GetId().c_str()
                                               encoding:[NSString defaultCStringEncoding]];
            key = [key stringByAppendingString:@"*"];
            int rowFactId = 0;
            for (auto fact : factSet->GetFacts()) {
                RichTextElementProperties titleTextProp{
                    [_hostConfig getHostConfig] -> GetFactSet().title,
                    fact->GetTitle(),
                    fact->GetLanguage()};
                [self processTextConcurrently:titleTextProp
                                    elementId:[key stringByAppendingString:
                                                       [[NSNumber numberWithInt:rowFactId++]
                                                           stringValue]]];

                RichTextElementProperties valueTextProp{
                    [_hostConfig getHostConfig] -> GetFactSet().value,
                    fact->GetValue(),
                    fact->GetLanguage()};
        ...
        case CardElementType::ImageSet: {
            ...
            for (auto img : imgSetElem->GetImages()) {  // loops through images in image set
                std::shared_ptr<BaseCardElement> baseImgElem =
                    std::static_pointer_cast<BaseCardElement>(img);
                ...
                ObserverActionBlock observerAction =
                    ^(NSObject<ACOIResourceResolver> *imageResourceResolver, NSString *key,
                      ...
                      if (view) {
                          ...
                          // store the image view and image set element for easy retrieval in
                          // ACRView::observeValueForKeyPath
                          [rootView setImageView:key view:view];
                          [rootView setImageContext:key context:elem];
                      }
                ...
                [self loadImageAccordingToResourceResolverIF:baseImgElem
                                                         key:nil
                                              observerAction:observerAction];
            }
            break;
        ...
        case CardElementType::Media: {
            ...
            if (!poster.empty()) {
                ObserverActionBlock observerAction =
                    ^(NSObject<ACOIResourceResolver> *imageResourceResolver, NSString *key,
                      std::shared_ptr<BaseCardElement> const &imgElem, NSURL *url,
                      ACRView *rootView) {
                      UIImageView *view = [imageResourceResolver resolveImageViewResource:url];
                      ...
                      if (view) {
                          ...
                          // store the image view and media element for easy retrieval in
                          // ACRView::observeValueForKeyPath
            ...
            if (![_hostConfig getHostConfig] -> GetMedia().playButton.empty()) {
                      ...
                      if (view) {
                          [view addObserver:rootView
                                 forKeyPath:@"image"
                                    options:NSKeyValueObservingOptionNew
                                    context:nil];
                          // store the image view for easy retrieval in
                          // ACRView::observeValueForKeyPath
                          [rootView setImageView:key view:view];
                      }
                    };
                ...
                NSNumber *number =
                    [NSNumber numberWithUnsignedLongLong:(unsigned long long)elem.get()];
                NSString *key =
                    [NSString stringWithFormat:@"%@_%@", [number stringValue], @"playIcon"];

                [self loadImageAccordingToResourceResolverIFFromString:[_hostConfig getHostConfig]
                                                                       -> GetMedia().playButton
                                                                   key:key
                                                        observerAction:observerAction];
            }
        ...
        case CardElementType::TextInput: {
            ...
            if (action != nullptr && !action->GetIconUrl().empty()) {
                ObserverActionBlockForBaseAction observerAction = ^(
                    NSObject<ACOIResourceResolver> *imageResourceResolver, NSString *key,
                    std::shared_ptr<BaseActionElement> const &elem, NSURL *url, ACRView *rootView) {
                  UIImageView *view = [imageResourceResolver resolveImageViewResource:url];
                  if (view) {
                      [view addObserver:self
                             forKeyPath:@"image"
                ...
                [self loadImageAccordingToResourceResolverIFForBaseAction:action
                                                                      key:nil
                                                           observerAction:observerAction];
            }
            break;
        ...
        case CardElementType::Container: {
            ...
            if ((backgroundImageProperties != nullptr) &&
                !(backgroundImageProperties->GetUrl().empty())) {
                ObserverActionBlock observerAction = generateBackgroundImageObserverAction(
                    backgroundImageProperties, self, container);
                [self loadBackgroundImageAccordingToResourceResolverIF:backgroundImageProperties
                                                                   key:nil
                                                        observerAction:observerAction];
            }
        ...
        case CardElementType::ColumnSet: {
            ...
            for (auto const &column : columns) {  // update serial number that is used for
                                                  // generating unique key for image_map
        ...
        case CardElementType::Column: {
            ...
            if ((backgroundImageProperties != nullptr) &&
                !(backgroundImageProperties->GetUrl().empty())) {
                ObserverActionBlock observerAction =
                    generateBackgroundImageObserverAction(backgroundImageProperties, self, column);
                [self loadBackgroundImageAccordingToResourceResolverIF:backgroundImageProperties
                                                                   key:nil
                                                        observerAction:observerAction];
            }
...
// Walk through adaptive cards elements recursively and if images/images set/TextBlocks are found
...
- (void)addTasksToConcurrentQueue:(std::vector<std::shared_ptr<BaseCardElement>> const &)body {
    ...
    for (auto &elem : body) {
        if ([rendererRegistration
                isElementRendererOverridden:(ACRCardElementType)elem->GetElementType()] == YES) {
            continue;
        }
...
- (void)loadImagesForActionsAndCheckIfAllActionsHaveIconImages:
            (std::vector<std::shared_ptr<BaseActionElement>> const &)actions
...
- (void)loadImage:(std::string const &)urlStr {
    ...
    NSObject<ACOIResourceResolver> *imageResourceResolver =
        [_hostConfig getResourceResolverForScheme:[url scheme]];
    ImageLoadBlock imageloadblock = nil;
    if (!imageResourceResolver ||
        ![imageResourceResolver respondsToSelector:@selector(resolveImageResource:)]) {
        imageloadblock = ^(NSURL *url) {
          // download image
          UIImage *img = nil;
          if ([url.scheme isEqualToString:@"data"]) {
              NSString *absoluteUri = url.absoluteString;
              std::string dataUri = AdaptiveCards::AdaptiveBase64Util::ExtractDataFromUri(
                  std::string([absoluteUri UTF8String]));
              std::vector<char> decodedDataUri = AdaptiveCards::AdaptiveBase64Util::Decode(dataUri);
              NSData *decodedBase64 = [NSData dataWithBytes:decodedDataUri.data()
                                                     length:decodedDataUri.size()];
        ...
        if (context) {
            ...
            if (baseCardElement) {
                ...
                if (renderer && [[renderer class] conformsToProtocol:@protocol(ACRIKVONotificationHandler)]) {
                    ...
                    [renderer configUpdateForUIImageView:baseCardElement
                                                  config:_hostConfig
                                                   image:image
                                               imageView:(UIImageView *)object];
                }
            } else {
...
- (void)loadBackgroundImageAccordingToResourceResolverIF:
    ...
    NSString *nSUrlStr = [NSString stringWithCString:backgroundImage->GetUrl().c_str()
                                            encoding:[NSString defaultCStringEncoding]];
...
- (void)loadImageAccordingToResourceResolverIF:(std::shared_ptr<BaseCardElement> const &)elem
    ...
    if (elem->GetElementType() == CardElementType::Media) {
        ...
        nSUrlStr = [NSString stringWithCString:mediaElem->GetPoster().c_str()
                                      encoding:[NSString defaultCStringEncoding]];
    } else {
        ...
        nSUrlStr = [NSString stringWithCString:imgElem->GetUrl().c_str()
                                      encoding:[NSString defaultCStringEncoding]];
    }
...
- (void)loadImage:(NSString *)nSUrlStr
               key:(NSString *)key
           context:(std::shared_ptr<BaseCardElement> const &)elem
    observerAction:(ObserverActionBlock)observerAction {
    _numberOfSubscribers++;
...
- (void)loadImageAccordingToResourceResolverIFForBaseAction:
            (std::shared_ptr<BaseActionElement> const &)elem
                                                        key:(NSString *)key
                                             observerAction:
                                                 (ObserverActionBlockForBaseAction)observerAction {
    NSNumber *number = nil;
    ...
    nSUrlStr = [NSString stringWithCString:elem->GetIconUrl().c_str()
                                  encoding:[NSString defaultCStringEncoding]];
    if (!key) {
        key = [number stringValue];
    }
...
- (void)updatePaddingMap:(std::shared_ptr<CollectionTypeElement> const &)collection
                    view:(UIView *)view {
...
- (void)updatePaddingMapForTopElements:(std::shared_ptr<BaseCardElement> const &)element
                              rootView:(ACRView *)view
                                  card:(std::shared_ptr<AdaptiveCard> const &)card {
    const CardElementType type = element->GetElementType();
    if (type == CardElementType::Container || type == CardElementType::ColumnSet ||
        type == CardElementType::Column) {
        std::shared_ptr<CollectionTypeElement> collection =
            std::dynamic_pointer_cast<CollectionTypeElement>(element);
        if (view && collection && collection->GetStyle() != card->GetStyle()) {
            NSNumber *key =
                [NSNumber numberWithUnsignedLongLong:collection->GetInternalId().Hash()];
            _paddingMap[[key stringValue]] = view;
        }
    }
}
...
- (void)processFallback:(std::shared_ptr<BaseCardElement> const &)elem {
    ...
    while (fallbackElem) {
        std::shared_ptr<BaseCardElement> fallbackElemCard =
            std::static_pointer_cast<BaseCardElement>(fallbackElem);
        if (fallbackElemCard) {
...
- (void)addWarnings:(ACRWarningStatusCode)statusCode mesage:(NSString *)message {
    [((NSMutableArray *)_warnings) addObject:[[ACOWarning alloc] initWith:statusCode
                                                                  message:message]];
}
