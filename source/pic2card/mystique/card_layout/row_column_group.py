...
def get_layout_structure(json_objects: List, queue: Queue) -> None:
    ...
    column_y_minimums = [
        design_object.get("ymin") for design_object in json_objects
    ]

    json_objects = [
        value
        for _, value in sorted(
            zip(column_y_minimums, json_objects), key=lambda value: value[0]
        )
    ]
...
def generate_card_layout(
    json_objects: List, image: Image, predict_card_object=None
) -> RowColumnGrouping:
    """
...
class RowColumnGroup:  # pylint: disable=too-few-public-methods
    """
    ...
    def row_column_grouping(
        ...
        for column_set in column_sets:  # pylint: disable=too-many-nested-blocks
            if len(column_set) == 1:
                ...
                if len(columns) == 1:
                    ...
                    for column in columns:
                        if len(column) == 1:
                            ...
                            row_columns = card_layout[row_counter]["row"]
                            if not self._check_same_iteration(
