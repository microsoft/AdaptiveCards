"""tests cases for predict_json api"""
import unittest
import json
import sys
...
class PredictJsonTestAPI(BaseAPITest):
    ...
    def test_response(self):
        ...
        self.assertIsNone(self.output["error"], msg="Key 'Error' is not 'null'")
    ...
    def test_image_max_size(self):
        ...
        response = get_response(
            self.client, self.api, self.headers, self.gt_2mb_data
        )
        output = json.loads(response.data)
    ...
    def test_empty_payload_data(self):
        ...
        response = get_response(
            self.client, self.api, self.headers, self.empty_data
        )
        output = json.loads(response.data)
    ...
    def test_wrong_payload_data(self):
        ...
        response = get_response(
            self.client, self.api, self.headers, self.wrong_data
        )
        output = json.loads(response.data)
        key = bool(output.get("error"))
        self.assertTrue(key, msg="Key 'error' is found")
