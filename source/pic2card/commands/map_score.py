...
def generate_map(
    ...
    """Generate mAP score for given model, use the
    given test dataset with labels."""
    ...
    _ = not os.path.exists(ground_truth_dir) and os.mkdir(ground_truth_dir)
    _ = not os.path.exists(pred_truth_dir) and os.mkdir(pred_truth_dir)
    ...
    for img_name in images:
        ...
        classes, scores, boxes = object_detection.get_bboxes(
            img_path, image_pipeline
        )
        ...
        for pred in pred_iter:
            ...
            if score > bbox_min_score:
                preds.append((label, score, bbox[0], bbox[1], bbox[2], bbox[3]))
        ...
        data_df[data_df.filename == img_name][columns].to_csv(
            f"{gt_dir}/{fname}.txt", header=False, sep=" ", index=False
        )
