...
class Helper:
    ...
    def action_set(self, element: Dict) -> Dict:
        ...
        element["actions"] = [dict(sorted(element["actions"][0].items(),
                                          key=lambda s: s[0]))]
        return element
    ...
    def choice_set(self, element: Dict) -> Dict:
        ...
        element["choices"] = [self.clean_element(ch)
                              for ch in element["choices"]]
        return element

    def clean_containers_train(self, body: List, element: Dict,
                               pre_process_object=None,
                               ground_truth=False) -> None:
        """
...
class Preprocess:
    ...
    def pre_process_elements(self, body: List, design_object: Dict) -> None:
        ...
        if (isinstance(design_object, dict) and
                design_object.get("type", "") not in self.CONTAINERS):
            ...
            self.helper_object.clean_containers_train(body, design_object,
    ...
    def process_elements_test(self, body: List, design_object: Dict) -> None:
        ...
        if (isinstance(design_object, dict) and
            ...
            design_object = dict(sorted(design_object.items(),
                                        key=lambda s: s[0]))
            body.append(design_object)
        elif isinstance(design_object, list):
            for design_obj in design_object:
    ...
    def main(self, api_url=None, images_path=None,
             ground_truths_path=None, debug=False) -> None:
        """
        ...
        for ctr, train_image in enumerate(images):
            ...
            score = corpus_bleu([[train_corpus.lower().split()]],
