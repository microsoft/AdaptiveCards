...
AdaptiveCard::AdaptiveCard(
    ...
    std::vector<std::shared_ptr<BaseActionElement>>& actions) :
    AdaptiveCard(
        version, fallbackText, backgroundImage, std::shared_ptr<Refresh>(), std::shared_ptr<Authentication>(), style, speak, language, verticalContentAlignment, height, minHeight, body, actions)
{
}
...
#ifdef __ANDROID__
std::shared_ptr<ParseResult> AdaptiveCard::DeserializeFromString(const std::string& jsonString, std::string rendererVersion) throw(AdaptiveCards::AdaptiveCardParseException)
#else
...
#else
std::shared_ptr<AdaptiveCard> AdaptiveCard::MakeFallbackTextCard(const std::string& fallbackText, const std::string& language, const std::string& speak)
#endif // __ANDROID__
{
