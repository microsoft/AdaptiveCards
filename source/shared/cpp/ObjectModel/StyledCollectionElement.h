...
public:
    ...
    StyledCollectionElement(CardElementType type, ContainerStyle style = ContainerStyle::None, std::optional<VerticalContentAlignment> alignment = std::nullopt);
    ...
    {
        return (m_bleedDirection != ContainerBleedDirection::BleedRestricted);
    }
...
private:
    ...
    void SetBleedDirection(const ContainerBleedDirection bleedDirection)
    {
...
std::shared_ptr<T> StyledCollectionElement::Deserialize(ParseContext& context, const Json::Value& value)
{
    ...
    collection->SetStyle(ParseUtil::GetEnumValue<ContainerStyle>(value, AdaptiveCardSchemaKey::Style, ContainerStyle::None, ContainerStyleFromString));
    ...
    if (const auto& minHeight = ParseSizeForPixelSize(ParseUtil::GetString(value, AdaptiveCardSchemaKey::MinHeight), &context.warnings);
        minHeight.has_value())
    {
...
template <typename T>
void StyledCollectionElement::GetResourceInformation(std::vector<RemoteResourceInformation>& resourceInfo, const std::vector<std::shared_ptr<T>>& elements)
{
