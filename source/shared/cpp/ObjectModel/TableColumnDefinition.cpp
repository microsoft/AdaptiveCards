...
std::string TableColumnDefinition::Serialize()
{
...
std::optional<unsigned int> TableColumnDefinition::GetWidth() const
{
    return m_width;
}
...
std::optional<unsigned int> TableColumnDefinition::GetPixelWidth() const
{
    return m_pixelWidth;
}
...
std::shared_ptr<TableColumnDefinition> TableColumnDefinition::Deserialize(ParseContext& context, const Json::Value& json)
{
    ...
    if (const auto& widthValue = ParseUtil::ExtractJsonValue(json, AdaptiveCardSchemaKey::Width, false); !widthValue.empty())
    {
        ...
        {
            if (const auto& pixelWidth = ParseSizeForPixelSize(widthValue.asString(), &(context.warnings)); pixelWidth.has_value())
            {
            ...
            else if (!pixelWidth.has_value())
            {
                context.warnings.emplace_back(std::make_shared<AdaptiveCardParseWarning>(
                    WarningStatusCode::InvalidDimensionSpecified, "Supplied value for \"width\" could not be parsed"));
            }
        }
        else
        {
            context.warnings.emplace_back(
                std::make_shared<AdaptiveCardParseWarning>(WarningStatusCode::InvalidValue, "Invalid type for \"width\""));
        }
    }
...
std::shared_ptr<TableColumnDefinition> TableColumnDefinition::DeserializeFromString(ParseContext& context, const std::string& jsonString)
{
