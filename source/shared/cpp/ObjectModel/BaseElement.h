...
public:
    BaseElement() :
        m_typeString{}, m_additionalProperties{}, m_requires{}, m_fallbackContent(nullptr), m_id{}, m_internalId{InternalId::Current()}, m_fallbackType(FallbackType::None), m_canFallbackToAncestor(false)
    {
    ...
    if (!fallbackValue.empty())
    {
        ...
        if (fallbackValue.isString())
        {
            ...
            throw AdaptiveCardParseException(
                ErrorStatusCode::InvalidPropertyValue, "The only valid string value for the fallback property is 'drop'.");
        }
