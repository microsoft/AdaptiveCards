...
TEST_CLASS(GatherImagesTest){private : void ValidateResourceInformation(
    std::vector<RemoteResourceInformation> & expectedValues,
    std::vector<RemoteResourceInformation>& resourceInfos){Assert::AreEqual(expectedValues.size(), resourceInfos.size());
...
for (auto expectedValue : expectedValues)
{
    ...
    {
        if (resourceInfo.url == expectedValue.url && resourceInfo.mimeType == expectedValue.mimeType)
        {
            found = true;
        }
    }
...
TEST_METHOD(CanGatherCustomImages)
{
    ...
    {
    public:
        TestCustomElement(const Json::Value& value) :
            BaseCardElement(AdaptiveCards::CardElementType::Custom), BaseActionElement(AdaptiveCards::ActionType::Custom)
        {
    ...
    // Parse the card and get the image uris
    auto resourceInformation =
        AdaptiveCard::DeserializeFromString(testJsonString, "1.0", context)->GetAdaptiveCard()->GetResourceInformation();
    ValidateResourceInformation(expectedValues, resourceInformation);
}
