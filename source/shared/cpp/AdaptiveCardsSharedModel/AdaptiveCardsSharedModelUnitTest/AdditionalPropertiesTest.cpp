...
TEST_CLASS(AdditionalPropertyTest){public : TEST_METHOD(CanGetAdditionalProperitesAtCardLevelTest){std::string testJsonString = "{\
...
TEST_METHOD(UnknownElementRoundtripping)
{
    ...
    const auto outputCard = parseResult->GetAdaptiveCard()->Serialize();
    Assert::AreEqual(
        "{\"actions\":[],\"body\":[{\"text\":\"Standard textblock\",\"type\":\"TextBlock\"},{\"property\":\"value\",\"someOtherProperty\":\"some other value\",\"type\":\"SomeRandomType\"}],\"type\":\"AdaptiveCard\",\"version\":\"1.0\"}\n"s,
        outputCard);
}
}
;
