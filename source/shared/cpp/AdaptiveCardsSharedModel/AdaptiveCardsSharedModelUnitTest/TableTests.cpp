...
TEST_CLASS(TableTests){public : TEST_METHOD(TableCellParse){const std::string tableCellFragment{R"(
            {
                "type": "TableCell",
                "items": [
                    {
...
Assert::AreEqual(
    "{\"items\":[{\"text\":\"flim\",\"type\":\"TextBlock\"},{\"text\":\"iz-us\",\"type\":\"TextBlock\"}],\"rtl\":true,\"type\":\"TableCell\"}\n"s,
    serializedResult,
    L"TableCell should roundtrip correctly");
} // namespace AdaptiveCardsSharedModelUnitTest
...
TEST_METHOD(TableRowParse)
{
    ...
    Assert::AreEqual(
        "{\"cells\":[{\"items\":[{\"text\":\"the first\",\"type\":\"TextBlock\"},{\"text\":\"the first part deux\",\"type\":\"TextBlock\"}],\"rtl\":true,\"type\":\"TableCell\"},{\"items\":[{\"text\":\"the second\",\"type\":\"TextBlock\"}],\"rtl\":true,\"type\":\"TableCell\"}],\"horizontalCellContentAlignment\":\"center\",\"style\":\"Accent\",\"type\":\"TableRow\",\"verticalCellContentAlignment\":\"Bottom\"}\n"s,
        serializedResult);
}
...
TEST_METHOD(TableElementsParserRegistration)
{
    ...
    Assert::IsNull(
        context.elementParserRegistration->GetParser("TableRow").get(), L"Should not be a registered parser for TableRow");
    Assert::IsNull(
        context.elementParserRegistration->GetParser("TableCell").get(), L"Should not be a registered parser for TableCell");
}
...
TEST_METHOD(TableColumnDefinitionPixelParse)
{
    ...
    auto serializedResult = columnDefinition->Serialize();
    Assert::AreEqual(
        "{\"horizontalCellContentAlignment\":\"right\",\"verticalCellContentAlignment\":\"Center\",\"width\":\"100px\"}\n"s, serializedResult);
}
...
TEST_METHOD(TableColumnDefinitionMissingUnitParse)
{
    ...
    auto columnDefinition = TableColumnDefinition::DeserializeFromString(context, columnDefinitionFragment);
    Assert::IsFalse(
        columnDefinition->GetWidth().has_value(), L"A string width with no units should not result in width getting set");
    Assert::IsFalse(
        columnDefinition->GetPixelWidth().has_value(),
        L"A string width with no units should not result in pixel width getting set");
...
TEST_METHOD(TableCardParseOrphanedTableRow)
{
    ...
    auto bodyElem = body.at(0);
    Assert::AreEqual(
        "TableRow"s, bodyElem->GetElementTypeString(), L"An orphaned TableRow should deserialize with its type string intact");
    Assert::AreEqual(
    ...
    auto serializedCard = card->Serialize();
    Assert::AreEqual(
        R"({"actions":[],"body":[{"cells":[{"items":[{"text":"...","type":"TextBlock"}],"type":"TableCell"}],"type":"TableRow"}],"type":"AdaptiveCard","version":"1.5"}
)"s,
        serializedCard);
}
...
TEST_METHOD(TableCardParseOrphanedTableCell)
{
    ...
    auto bodyElem = body.at(0);
    Assert::AreEqual(
        "TableCell"s, bodyElem->GetElementTypeString(), L"An orphaned TableCell should deserialize with its type string intact");
    Assert::AreEqual(
        CardElementTypeToString(CardElementType::Unknown),
        CardElementTypeToString(bodyElem->GetElementType()),
    ...
    auto serializedCard = card->Serialize();
    Assert::AreEqual(
        R"({"actions":[],"body":[{"items":[{"text":"Name","type":"TextBlock","weight":"Bolder","wrap":true}],"type":"TableCell"}],"type":"AdaptiveCard","version":"1.5"}
)"s,
        serializedCard);
}
