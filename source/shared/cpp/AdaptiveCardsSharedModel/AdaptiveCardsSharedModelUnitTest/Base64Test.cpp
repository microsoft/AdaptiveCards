...
TEST_CLASS(Base64Test){public :

                           bool ContainSameCharacters(const std::string& s, const std::vector<char>& v){if (s.size() != v.size()){return false;
}
...
TEST_METHOD(DecodingTest)
{
    ...
    Assert::IsTrue(
        expectedDecodedData[0].size() == AdaptiveBase64Util::Decode(std::string(encodedData[0].begin(), encodedData[0].end())).size());

    for (size_t i{1}; i < encodedData.size(); ++i)
    {
        Assert::IsTrue(ContainSameCharacters(
            expectedDecodedData[i], AdaptiveBase64Util::Decode(std::string(encodedData[i].begin(), encodedData[i].end()))));
    }
}
...
TEST_METHOD(FailToDecodeTest)
{
    std::vector<std::string> badUri{
        "foo_bar", "foo(bar)", "foo-bar", "foo*bar", "foo\"bar", "foo&bar", "foo^bar", "foo#bar", "foo@bar", "foo!bar"};
