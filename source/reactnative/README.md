...
**ref** | `React.createRef()` |  Reference variable used to invoke the methods exposed by AdaptiveCards class.(Example: In order to fetch the image & media URLs across the payload, one can use like this this.refs.adaptiveCardRef.getResourceInformation())  | NO
