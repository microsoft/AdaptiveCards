...
namespace AdaptiveCards.Templating
{
    ...
    public sealed class AdaptiveCardsTemplateVisitor : AdaptiveCardsTemplateParserBaseVisitor<AdaptiveCardsTemplateResult>
    {
        ...
        /// <param name="rootData">json data as string which will be set as a root data context</param>
        /// <param name="hostData">json data as string which will be set as the host data context</param>
        public AdaptiveCardsTemplateVisitor(Func<string, object> nullSubstitutionOption, string rootData = null, string hostData = null)
        {
            ...
            if (!String.IsNullOrEmpty(rootData))
            {
                // set data as root data context
                try
                {
                    using (var jsonReader = new JsonTextReader(new StringReader(rootData)) { DateParseHandling = DateParseHandling.None })
                    {
                        root = JToken.Load(jsonReader);
                        PushDataContext(rootData, root);
                    }
                }
                catch (JsonException innerException)
                {
