...
TypeAheadSearchCustomUIParams - Host provides custom icons (search, cross icons etc.) and error text information though this data class
...
4. Call `updateUI` method once the response is received from the host.
...
Response type will be an enum with these values :error,offline,success

E.g., if the user had typed "Ma" it would return something like (NSArray<ChoiceInput *> choicesArray):
```
[
   { "title": "Matt", "value": "1" },
   { "title": "Mark", "value": "2" }
...
```
if there is an error while querying the bot then it would return something like:
```
Code=204,
...
`ACRChoiceSetTypeaheadSearchView` will update the `_searchStateTitleLabel` and `_searchStateImageView` with the default or user provided info.

Type ahead search UI states - 

1. zeroSearch - Default state when new screen is opened and there are no static choices to be displayed.
