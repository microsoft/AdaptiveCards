...
- The parser will raise parse errors if required properties like type and dataset are missing. Host app can choose whether to attempt rendering or not, despite parse warning
- We do assign default values to optional properties in the adaptive card and will also assign default values to optional properties in the choice.data (For eg. skip and count are optional in choices.data). We will set default values of skip (0) and count (15).

Rendering

- If dataset or type property in choices.data is not given, is null, is empty, or has invalid value we will raise parsing error and skip rendering of dynamic typeahead search control.
